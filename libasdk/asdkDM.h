...
/* Class prototype */
class IDeformableMirrorImplementation;
...
public:
     ...
     * 
     * @param[in] value Array of values to send to the DM.
    ...
    /** Set all actuators (modes for DMMs) to the value zero.
     *
     * @return FAILURE in case of failure, SUCCESS otherwise.
     */
    COMPL_STAT Reset();
     ...
     *                   of actuators (or modes for DMM) multiplied by the
     *                   number of patterns.
     * @param[in] nPattern Number of patterns to be sent.
     * @param[in] nRepeat  Number of time to repeat that pattern or 0 for infinite
     *                   (some interface does not allow you to use this feature).
     *
     * @return FAILURE in case of failure, SUCCESS otherwise.
     */
    COMPL_STAT Send( const Scalar * values, UInt nPattern, UInt nRepeat = 1 );

    /** @brief Send patterns on one actuator.
     *  @deprecated This method use deprecated functionalities, please use Send.
     */
    COMPL_STAT SendOne( const Scalar *, UInt, UInt, UInt );
...
public:
     ...
     *            a list of allowed parameters.
     *            
     * @param[out] the function allocates a new vector, then copy
     *             the pointer to vector. This vector must be
     *             deallocated.
     *
     * @return If found, return the size of the vector
     *         If not found, return 0 and Check() will return false.
     */
    Int Get( CStrConst command, Scalar** vector ) const;
    std::string GetString(CStrConst command) const;
     ...
     * @param[in] command Parameter name, see the documentation for
     *            a list of allowed parameters.
     * @param[in] value   Parameter value (numeric, or vector of numeric).
     *
...
private:
    IDeformableMirrorImplementation* _dmImplem;
};
